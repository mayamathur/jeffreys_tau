...
mybayesmeta <- function(y,...)
{
  UseMethod("mybayesmeta")
}


mybayesmeta.default <- function(y, sigma, labels=names(y),
                              tau.prior="uniform",
                              ...
                              tol.uniroot=rel.tol.integrate,...)
{
  ...
  if (is.character(tau.prior)) {
    tau.prior <- match.arg(tolower(tau.prior),
                           c("uniform","jeffreys", "jeffreys2","shrinkage","dumouchel","bergerdeely","conventional","i2","sqrt"))
    tau.prior <- c("uniform"="uniform", "jeffreys"="Jeffreys", "jeffreys2"="Jeffreys2",
                   "shrinkage"="shrinkage", "dumouchel"="DuMouchel",
                   "bergerdeely"="BergerDeely", "conventional"="conventional", "i2"="I2", "sqrt"="sqrt")[tau.prior]
    stopifnot(is.element(tau.prior, c("uniform", "Jeffreys", "Jeffreys2", "shrinkage", "DuMouchel", "BergerDeely", "conventional", "I2", "sqrt")))
    if (tau.prior=="uniform") {             # uniform prior on tau:
      ...
      ## MY PRIOR
    } else if (tau.prior=="Jeffreys2") {     # Jeffreys prior:
      pdens <- function(t){return(apply(matrix(t,ncol=1),1,function(x){
        Si = sqrt(sigma^2 + x^2)  # vector of length k
        return( x * sqrt( sum( Si^(-2) ) * sum( Si^(-4) ) ) ) 
        } ) ) } 
      attr(pdens, "bayesmeta.label") <- "Jeffreys2 prior"
